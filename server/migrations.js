...
Migrations.add({
  version: 18,
  name: "Final catch-all to ensure all tenants have default opportunity stages/project types",
  up: function() {
    ServerSession.set('maintenance', true);
    Partitioner.directOperation(function() {
      var tenants = Tenants.find({}).fetch();
      ...
      _.each(tenants, function(t) {
        ...
        if (t.settings.opportunity.stages.length === 0) {
          var defaultStages = [{
            title: "Exploration",
        ...
        if (t.settings.project.types.length === 0) {
          var sample = [];
          var projectType = {
            id: 0,
            name: "Standard Project",
