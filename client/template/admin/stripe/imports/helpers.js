...
export function updateUpcomingInvoice(self) {
  ...
  if(tenant.stripe.stripeId) {
    Meteor.call('stripe.getUpcomingInvoice', function(error, invoice) {
      ...
      let upcomingInvoice = _.cloneDeep(invoice);
      ...
      let tot = upcomingInvoice.lines.data.length;
      let i = 0;
      ...
      if(upcomingInvoice.lines.data[tot-1].description) {
        ...
        let periodStart = moment(upcomingInvoice.lines.data[tot-1].period.start*1000).format('DD/MM/YYYY');
        let periodEnd = moment(upcomingInvoice.lines.data[tot-1].period.end*1000).format('DD/MM/YYYY');
        newData.push({
...
export function updateLastInvoice(self) {
  ...
  if(tenant.stripe.stripeId) {
    Meteor.call('stripe.getLastInvoice', function(error, invoice) {
      ...
      let lastInvoice = _.cloneDeep(invoice);
